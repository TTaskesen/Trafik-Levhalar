--------------------------------------------------------------------------------
--
-- sahne3.lua - Trafik Tanzim İşaretleri
--
--------------------------------------------------------------------------------
...
-- tanzim.png (700x2439) içindeki 61 levhanın çerçeveleri:
-- İlk 60 levha 16 satır x 4 sütunluk kılavuzda, her hücre 150 piksel yüksekliğinde
-- (işaret + altındaki yazı birlikte alınır).
-- Son satır (61. levha) TT-43 işaretidir: tüm genişliği kaplayan tek levha.
local levhaTanzimKareleri = {
    sheetContentWidth = 700,
    sheetContentHeight = 2439,
    frames = {}
}
local tanzimSutunGenislikleri = { 175, 180, 170, 175 }
for i = 1, 60 do
    local satir = math.floor((i - 1) / 4)
    local sutun = (i - 1) % 4
    local x = 0
    for s = 1, sutun do
        x = x + tanzimSutunGenislikleri[s]
    end
    levhaTanzimKareleri.frames[i] = {
        x = x,
        y = satir * 150,
        width = tanzimSutunGenislikleri[sutun + 1],
        height = 150
    }
end
levhaTanzimKareleri.frames[61] = { x = 0, y = 2250, width = 700, height = 189 }

local resimLevha = graphics.newImageSheet("levha/levha/2-tanzim/tanzim.png",
    levhaTanzimKareleri)
...
function scene:create(event)
    ...
    -- Toplam 61 tanzim işareti var (1 kategori + 61 levha satırı)
    for i = 1, 62 do
